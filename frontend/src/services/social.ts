...
export const socialService = {
  async getFeed(limit = 20, offset = 0): Promise<Post[]> {
    try {
      const response = await axios.get('/social/posts', {
        params: { limit, offset }
      })
      // 适配新的响应格式
  ...
  async getPost(postId: string): Promise<Post> {
    const response = await axios.get(`/social/posts/${postId}`)
    return response.data.data || response.data
  },

  async deletePost(postId: string): Promise<void> {
    await axios.delete(`/social/posts/${postId}`)
  },
  ...
  async getComments(postId: string, limit = 50, offset = 0): Promise<Comment[]> {
    const response = await axios.get(`/social/posts/${postId}/comments`, {
      params: { limit, offset }
    })
    // 适配新的响应格式
  ...
  async uploadMedia(file: File): Promise<{ url: string }> {
    ...
    const response = await axios.post('/social/posts/media', formData, {
  ...
  async followUser(userId: number): Promise<void> {
    await axios.post(`/social/users/${userId}/follow`)
  },
  ...
  async getUserPosts(userId: number, limit = 20, offset = 0): Promise<Post[]> {
    const response = await axios.get(`/social/users/${userId}/posts`, {
      params: { limit, offset }
    })
    return response.data.data?.posts || response.data.posts || []
  },
  ...
  async getPopularTags(limit = 20): Promise<Array<{ tag: string; count: number }>> {
    const response = await axios.get('/social/tags', {
      params: { limit }
    })
